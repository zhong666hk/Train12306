  ...
  <a-modal v-model:visible="visible" title="每日车次" @ok="handleOk"
    ...
    <a-form :model="daily_train" :label-col="{span: 4}" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item label="车次编号">
        <a-input v-model:value="daily_train.code" />
      </a-form-item>
      ...
      <a-form-item label="始发站">
        <a-input v-model:value="daily_train.start" />
      </a-form-item>
      <a-form-item label="始发站拼音">
        <a-input v-model:value="daily_train.startPinyin" />
      </a-form-item>
      <a-form-item label="出发时间">
        <a-time-picker v-model:value="daily_train.startTime" valueFormat="HH:mm:ss" placeholder="请选择时间" />
      </a-form-item>
      <a-form-item label="终点站">
        <a-input v-model:value="daily_train.end" />
      </a-form-item>
      <a-form-item label="终点站拼音">
        <a-input v-model:value="daily_train.endPinyin" />
      </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    const onDelete = (record) => {
      axios.delete("/daily_train/admin/daily_train/delete/" + record.id).then((response) => {
    ...
    const handleOk = () => {
      axios.post("/daily_train/admin/daily_train/save", daily_train.value).then((response) => {
        if (response.code===200) {
          notification.success({description: response.message});
          visible.value = false;
          handleQuery({
    ...
    const handleQuery = (param) => {
      ...
      loading.value = true;
      axios.get("/daily_train/admin/daily_train/query-list", {
        params: {
          page: param.page,
          size: param.size
        }
